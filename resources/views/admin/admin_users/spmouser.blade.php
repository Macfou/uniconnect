...
@endif
        ...
        <div class="relative flex flex-col w-full h-full text-slate-700 bg-white mx-w-lg shadow-md rounded-xl">
            <div class="relative mx-4 mt-4 flex justify-between items-center">
                ...
                <h3 class="text-lg font-bold text-slate-800">SPMO</h3>
            ...
            <table class="w-full mt-4 text-left table-auto border border-slate-200 rounded-lg">
                ...
                <tbody>
                    @forelse ($spmoUsers as $spmo)
                        <tr class="hover:bg-slate-100">
                            <td class="p-4 border-b">{{ $spmo->fname }} {{ $spmo->lname }}</td>
                            <td class="p-4 border-b">{{ $spmo->email }}</td>
                            <td class="p-4 border-b">{{ $spmo->created_at->format('Y-m-d') }}</td>
                            <td class="p-4 border-b">
                               
                               
                
                                <!-- Delete Button -->
                                <form action="{{ route('admin.admin_users.spmouser.destroy', $spmo->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
...
<div id="deleteModal" class="modal hidden fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center">
    <div class="bg-white p-6 rounded shadow-lg w-96">
        <h2 class="text-lg font-medium mb-4">Are you sure you want to delete this <strong> {{ $spmo->fname }} {{ $spmo->lname }}</strong>?</h2>
        <p class="text-gray-600">This action cannot be undone.</p>
        <div class="flex justify-center space-x-4 mt-4">
            <!-- The action form for deleting the SPMO -->
            <form id="deleteForm" action="{{ route('admin.admin_users.spmouser.destroy', $spmo->id) }}" method="POST">
                @csrf
                @method('DELETE')
            ...
            <a href="#" class="py-2 px-4 bg-gray-500 text-white rounded hover:bg-gray-700">Cancel</a>
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="p-4 text-center">No SPMO Officers found.</td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        <div id="addFacilityModal" class="modal">
            <div class="modal-content">
                ...
                <h2 class="text-lg font-bold mb-4">Add SPMO</h2>
                <form action="{{ route('admin.admin_users.spmouser.store') }}" method="POST" class="space-y-4">
                    @csrf
                    <div>
