    ...
    <div class="pt-28 pb-32 px-6 max-w-7xl mx-auto">
        ...
        <div class="bg-white shadow-lg rounded-lg p-8">
            <h2 class="text-xl font-semibold text-gray-800 mb-2">
                Request Adviser Approval for: <span class="text-blue-600">{{ $event->tags ?? 'Untitled Event' }}</span>
            </h2>

            <a href="{{ route('view_adviser', ['id' => $event->id]) }}" 
                class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md text-sm font-medium transition">
                View Requests
             </a>

            <h2 class="text-2xl font-semibold text-center mb-6 text-gray-800">Search for Adviser by Email</h2>
            @if(session('success'))
...
@endif
            ...
            <div id="userInfo" class="mt-6 hidden">
                <h3 class="text-xl font-semibold text-gray-800 mb-3">Adviser Information:</h3>
            ...
            <!-- Form to submit the adviser -->
            <form id="approveForm" action="{{ route('adviser.approval.store') }}" method="POST" class="mt-6 space-y-6 hidden">
                @csrf
    ...
    <script>
        document.getElementById('searchForm').addEventListener('submit', function(event) {
            ...
            fetch('{{ route('searchadviser.user') }}', {
                method: 'POST',
                headers: {
            ...
            .then(data => {
                if (data.error) {
                    ...
                    document.getElementById('adviser_id').value = data.id;
