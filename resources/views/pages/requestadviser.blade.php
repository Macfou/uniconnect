        ...
        <div class="bg-white shadow-md rounded-xl overflow-hidden">
            <div class="p-6 border-b flex items-center justify-between">
                <div>
                    ...
                    <p class="text-sm text-gray-500">View all submitted requests and adviser approvals</p>
                </div>
            
                <div class="flex space-x-2">
                    <a href="{{ route('adviser_rejected') }}" 
                ...
                <table class="min-w-full divide-y divide-gray-200 text-sm">
                    <thead class="bg-gray-100 text-left text-xs font-semibold uppercase text-gray-600">
                        <tr>
                            ...
                            <th class="px-6 py-3">Adviser Approval</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4">
                                <form action="{{ route('adviser_approved', ['id' => $request->id]) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                                    <button class="text-blue-500 hover:underline" type="submit">Approve</button>
                                </form>
                                <form action="{{ route('adviser_reject', ['id' => $request->id]) }}" method="POST" class="inline">
                                    @csrf
                                    @method('PATCH')
                                    <button 
                                        type="submit"
                ...
                <div id="eventModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 hidden z-50">
                    <div class="bg-white p-6 w-[900px] rounded-lg shadow-lg border border-gray-300 max-h-screen overflow-y-auto">
                        <div id="modalContent">
                            ...
                            <p class="mt-4">Dear :<strong> <span id="modalCollege"></strong> Adviser </p>
            ...
            fetch(`/requests/check-email/${email}`)
                ...
                .then(data => {
                    if (data.status === 'exists') {
                        alert(`Adviser Approval Found: ${data.adviser.name}`);
                    } else {
                        alert('Adviser Approval Not Found');
                    }
                })
                .catch(error => {
