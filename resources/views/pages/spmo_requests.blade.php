<x-layout>
    ...
    <div class="max-w-6xl mx-auto mt-10 p-6 bg-white shadow-md rounded-lg">
        <h2 class="text-xl font-bold text-gray-800">My Equipment Requedvfddsts</h2>
                ...
                @foreach($requests as $request)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="p-4 border-b">
                            <span class="px-2 py-1 text-white rounded {{ strtolower($request->status) == 'approved' ? 'bg-green-500' : 'bg-yellow-500' }}">
                                {{ ucfirst($request->status) }}
                            </span>
                        </td>
                        <td class="p-4 border-b">
                            @if(strtolower($request->status) == 'pending')
                                <span class="text-yellow-600 font-semibold">Waiting for Approval</span>
                            @elseif(strtolower($request->status) == 'approved')
                                <span class="text-green-600 font-semibold">Ready for Pick Up</span>
                            @else
                                <span class="text-gray-600 font-semibold">{{ ucfirst($request->status) }}</span>
                            @endif
                        </td>
                    </tr>
                @endforeach
