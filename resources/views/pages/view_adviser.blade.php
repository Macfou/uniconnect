<x-layout>
    ...
    <h2 class="text-xl font-bold">Adviser Requests for: {{ $event->tags }}</h2>
...
<table class="min-w-full bg-white border mt-4">
    ...
    <tbody>
        ...
        <tr class="border-t">
            <td class="px-4 py-2">
                {{ $request->adviser->fname ?? 'N/A' }} {{ $request->adviser->lname ?? '' }}
            </td>
            <td class="px-4 py-2">
                {{ $request->adviser->email ?? 'N/A' }}
            </td>
            <td class="px-4 py-2">
                {{ $request->status ?? 'N/A' }}
            </td>
    ...
    @empty
            <tr>
                <td colspan="2" class="text-center py-4 text-gray-500">No adviser requests found.</td>
            </tr>
        @endforelse
    </tbody>
</table>
